...
public static class PrefabLoader
{
    ...
    public static List<T> LoadAllPrefabsOfType<T>(string path, SearchOption searchOption = SearchOption.TopDirectoryOnly) where T : MonoBehaviour
    {
        if (path != "")
        {
            if (path.EndsWith("/"))
        ...
        FileInfo[] fileInf = dirInfo.GetFiles("*.prefab", searchOption);
